            ...
            <div class="card mb-4">
                ...
                <div class="card-body">
                    <form action="{{ route('superadmin.project.update', $project->id) }}" method="POST">
                            ...
                            <div class="col-sm-10">
                                <select class="form-select @error('id_eps') is-invalid @enderror" id="id_eps"
                                    ...
                                    <option value="{{ $eps->id_eps }}"
                                        {{ (old('id_eps', $project->id_eps) == $eps->id_eps) ? 'selected' : '' }}
                                        {{ !old('id_eps') && !$project->id_eps && $defaultEps && $eps->id_eps == $defaultEps->id_eps ? 'selected' : '' }}>
                        ...
                        <div class="row mb-3">
                            <label class="col-sm-2 col-form-label" for="kode_rkap">Kode RKAP <span
                                    class="text-danger">*</span></label>
                            <div class="col-sm-10">
                                <input type="text" class="form-control @error('kode_rkap') is-invalid @enderror"
                                    id="kode_rkap" name="kode_rkap" value="{{ old('kode_rkap', $project->kode_rkap) }}"
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-sm-10">
                                <select class="form-select @error('id_disiplin') is-invalid @enderror" id="id_disiplin"
                                    ...
                                    <option value="{{ $disiplin->id_disiplin }}" {{ (old('id_disiplin', $project->id_disiplin) == $disiplin->id_disiplin) ? 'selected' : '' }}>
                                        {{ $disiplin->remark }}
                                    </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-sm-10">
                                <select class="form-select @error('id_sub_disiplin') is-invalid @enderror"
                                    id="id_sub_disiplin" name="id_sub_disiplin">
                                    <option value="">Pilih Sub Disiplin</option>
                                    @foreach($sub_disiplin_list as $sub_disiplin)
                                    <option value="{{ $sub_disiplin->id_sub_disiplin }}"
                                        {{ (old('id_sub_disiplin', $project->id_sub_disiplin) == $sub_disiplin->id_sub_disiplin) ? 'selected' : '' }}>
                                        {{ $sub_disiplin->remark }}
                                    </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-sm-10">
                                <input type="text" class="form-control @error('tagno') is-invalid @enderror" id="tagno"
                                    name="tagno" value="{{ old('tagno', $project->tagno) }}"
                                    placeholder="Masukkan Tag No" required />
                                @error('tagno')
                                <div class="invalid-feedback">
                                    {{ $message }}
                                </div>
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-sm-10">
                                <input type="text" class="form-control @error('remark') is-invalid @enderror"
                                    id="remark" name="remark" value="{{ old('remark', $project->remark) }}"
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-sm-10">
                                <input type="number" class="form-control @error('weight_factor') is-invalid @enderror"
                                    ...
                                    value="{{ old('weight_factor', $project->weight_factor) }}" step="0.01" min="0"
                                    placeholder="Masukkan Weight Factor" required />
                                @error('weight_factor')
                                <div class="invalid-feedback">
                                    {{ $message }}
                                </div>
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-sm-10">
                                ...
                                @php
                                    $currentKategori = old('kategori', $project->kategori);
                                @endphp
                                {{-- <small class="text-muted">Current value: "{{ $currentKategori }}"</small> --}}
                                ...
                                <select class="form-select @error('kategori') is-invalid @enderror" id="kategori"
                                    ...
                                    @php
                                        // Make sure to use strict string comparison with the same case and no whitespace
                                        $isSelected = strcasecmp(trim($currentKategori ?? ''), trim($kategori ?? '')) === 0;
                                        // Fallback comparison in case strcasecmp doesn't work
                                        if (!$isSelected) {
                                            $isSelected = strtolower(trim($currentKategori ?? '')) === strtolower(trim($kategori ?? ''));
                                        }
                                    @endphp
                                    <option value="{{ $kategori }}" {{ $isSelected ? 'selected' : '' }}>
                                        {{ $kategori }}
                                    </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="row">
                            @for ($i = 1; $i <= 15; $i++)
                                ...
                                <input type="date"
                                    ...
                                    id="step_{{$i}}_date"
                                    name="step_{{$i}}_date"
                                    value="{{ old('step_'.$i.'_date', $project->{'step_'.$i.'_date'} ? $project->{'step_'.$i.'_date'}->format('Y-m-d') : '') }}" />
                                @error('step_'.$i.'_date')
                                <div class="invalid-feedback">
                                    {{ $message }}
                                </div>
                ...
                <div class="row justify-content-end mt-3">
                    <div class="col-sm-10">
                        ...
                        <a href="{{ route('superadmin.project') }}" class="btn btn-link">Kembali</a>
                    </div>
                </div>
                </form>
            </div>
        ...
        function loadSubDisiplin(idDisiplin) {
            ...
            fetch(`{{ route('superadmin.master-sub-disiplin.get-by-disiplin', ['id_disiplin' => ':id_disiplin']) }}`.replace(':id_disiplin', idDisiplin))
                ...
                .then(data => {
                    ...
                    data.forEach(item => {
                        const selected = item.id_sub_disiplin == "{{ old('id_sub_disiplin', $project->id_sub_disiplin) }}" ? 'selected' : '';
