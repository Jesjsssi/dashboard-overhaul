...
@section('content')
    <div class="container-xxl flex-grow-1 container-p-y">
        <h4 class="fw-bold py-3 mb-4">
            <span class="text-muted fw-light">EPS /</span> Edit EPS
        </h4>
            ...
            <div class="col-md-12">
                <div class="card mb-4">
                    <div class="card-header d-flex align-items-center justify-content-between">
                        <h5 class="mb-0">Form Edit EPS</h5>
                    </div>
                    <div class="card-body">
                        <form action="{{ route('superadmin.eps.update', $eps->id_eps) }}" method="POST">
                            @csrf
                            @method('PUT')
                            <div class="row mb-3">
                                <label class="col-sm-2 col-form-label" for="jenis_project">Jenis Project</label>
                                <div class="col-sm-10">
                                    <select class="form-select @error('jenis_project') is-invalid @enderror"
                                        ...
                                        @foreach(App\Models\EPS::getJenisProjectList() as $value => $label)
                                            <option value="{{ $value }}" {{ old('jenis_project', $eps->jenis_project) == $value ? 'selected' : '' }}>
                                                {{ $label }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="row mb-3">
                                ...
                                <div class="col-sm-10">
                                    <input type="text" class="form-control @error('remark') is-invalid @enderror"
                                        id="remark" name="remark" value="{{ old('remark', $eps->remark) }}"
                            ...
                            <div class="row mb-3" id="execution_date_container">
                                ...
                                <div class="col-sm-10">
                                    <input type="date" class="form-control @error('execution_date') is-invalid @enderror"
                                        ...
                                        value="{{ old('execution_date', $eps->execution_date ? date('Y-m-d', strtotime($eps->execution_date)) : '') }}" />
                            ...
                            <div class="row mb-3">
                                ...
                                <div class="col-sm-10">
                                    <input type="number" class="form-control @error('tahun') is-invalid @enderror"
                                        id="tahun" name="tahun" value="{{ old('tahun', $eps->tahun) }}"
                            ...
                            <div class="row mb-3">
                                ...
                                <div class="col-sm-10">
                                    <input type="date" class="form-control @error('cutoff_date') is-invalid @enderror"
                                        id="cutoff_date" name="cutoff_date" value="{{ old('cutoff_date', $eps->cutoff_date ? date('Y-m-d', strtotime($eps->cutoff_date)) : '') }}"
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Set placeholder awal sesuai jenis project yang dipilih
    const remarkInput = document.getElementById('remark');
    if (jenisProjectSelect.value === 'Overhaul') {
        remarkInput.placeholder = 'Masukkan keterangan Overhaul';
    } else if (jenisProjectSelect.value === 'Routine') {
        remarkInput.placeholder = 'Masukkan keterangan Routine';
    }
});
</script>
