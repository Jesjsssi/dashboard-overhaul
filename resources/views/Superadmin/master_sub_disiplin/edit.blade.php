        ...
        <div class="col-lg-12 mb-4 order-0">
            <div class="card">
                <div class="d-flex align-items-end row">
                    <div class="col-12">
                        <div class="card-body">
                            <h5 class="card-title text-primary">Edit Sub Disiplin</h5>
                            <p class="mb-4">Edit data sub disiplin</p>

                            <form action="{{ route('superadmin.master-sub-disiplin.update', $subDisiplin->id_sub_disiplin) }}" method="POST">
                                @csrf
                                @method('PUT')
                                <div class="row">
                                    <div class="col-md-6">
                                        <div class="mb-3">
                                            ...
                                            <input type="text" class="form-control @error('remark') is-invalid @enderror" 
                                                   id="remark" name="remark" value="{{ old('remark', $subDisiplin->remark) }}" required>
                                            @error('remark')
                                            ...
                                            <select class="form-select @error('id_disiplin') is-invalid @enderror" 
                                                ...
                                                @foreach($disiplins as $disiplin)
                                                    <option value="{{ $disiplin->id_disiplin }}" 
                                                            {{ old('id_disiplin', $subDisiplin->id_disiplin) == $disiplin->id_disiplin ? 'selected' : '' }}>
                                                        {{ $disiplin->remark }}
                                                    </option>
                                                @endforeach
                                            </select>
